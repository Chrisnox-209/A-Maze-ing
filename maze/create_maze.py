...
def create_maze(maze):
    ...
    while stack:
        ...
        y = cell_work[1]
        cell = maze.grid[y][x]
        if Theme.animation_draw == True and Theme.color_animation_backtraking is not None:
        ...
        for direct_x, direct_y in direction:
            if 0 <= direct_x < maze.width and 0 <= direct_y < maze.height:
                ...
                if verif_visit.visit == True:
                    continue
                if verif_visit.visit == False:
                    found = True
